...
//Rare exceptions to indicate a bug in the _avf_daroshka_022 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_avf_daroshka_022 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_avf_daroshka_022]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.avf_daroshka_022.module"
